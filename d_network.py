...
def get_batches(TrainData, mini_batch_size, hideLayerNum=30):
    ...
    for k in xrange(0, n_train, mini_batch_size*3/4):
...
class Network(object):
    ...
    def SGD(self, training_data, training_data_for_test, epochs, mini_batch_size, eta,
        ...
        n = len(training_data)
        self.val_errorRate = []  # 测试错误率
        self.train_errorRate = []  # 训练错误率
        for j in xrange(epochs):
            mini_batches = get_batches(training_data, mini_batch_size)  # 与BGD不同的地方
            ...
            if test_data:
                ...
                print "Epoch {0}: {1} / {2}".format(
                    j, val_num_right, n_test)
                self.val_errorRate.append((n_test-val_num_right)*1.0/n_test)
            else:
                print "Epoch {0} complete".format(j)
        ErrorRatePath = './cache/fixedAllPar_HL50_Epoch800/ErrorRatecrossbatchfixed.pkl'
        # if len(mini_batches) <= n/mini_batch_size + 1:
        #     Description = ['the batches generated by traditional ways']
        # else:
        #     Description = ['the batches generated by new ways']
        # pickle.dump(Description, open(ErrorRatePath, 'w'))
